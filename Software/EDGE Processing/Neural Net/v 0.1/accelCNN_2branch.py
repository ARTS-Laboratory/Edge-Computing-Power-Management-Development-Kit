...
Two-branch CNN for raw PATCH PPG + XYZ accelerometer.

Training:
3969, 4021, 4045, 4083, 4084, 4555, 4737, 4740
...
Runs 5 independent training runs using fixed seeds.
"""
...
class TwoBranchCNN(nn.Module):
    ...
    def forward(self, x):
        ppg = x[:, 0:1, :]
        accel = x[:, 1:4, :]
        ppg_features = self.ppg_branch(ppg)
        accel_features = self.accel_branch(accel)
        combined = torch.cat((ppg_features, accel_features), dim=1)
...
def main(train_ids, validation_ids, test_ids):
    ...
    output_file = "CNN_ACCEL_2branch_5run_results.csv"
